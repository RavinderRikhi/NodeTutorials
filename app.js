...
//.get method states that the HTTP method for which the following function will respond to will be a GET request
//The first parameter will help in routing 
app.get("/",function(req , res){	//No content type needed to be stated in express. It auto detects the body and attach the content type to the response
	res.send('<html><head>Express Server</head><body><h1>Welcome User...</h1></body</html>');//
});

app.get("/api",function(req,res){
	res.json({firstname:'John',lastname:'Doe'});//pass a JSON object and it will transform it to a string and pass it as body alongside the header file containing content type as Application/JSON 
});
...
// Express routing(Passing variables)
...
app.get("/",function(req , res){	
	res.send('<html><head></head><body><h1>Welcome User...</h1></body</html>');//
});

app.get("/person/:number",function(req , res){	//here the string used after a colon(:) will act as a variable which will carry the string value present at that place when the request is sent
	res.send('<html><head></head><body><h1>Person : '+req.params.number+'</h1></body</html>');//the value inside variable number mentioned in the request can be accessed inside the req.params object with same name i.e req.params.number 
});
...
//Static files the files that aren't processed or generated by code in any way e.g. CSS files, images ,etc which are uploaded to the server & are just sitting there
//These files are still need to be delivered when requested via an HTTP request although these files aren't connected to the request but server still has to decide what to do with it
...
//Using a middleware will help us to download static files(style.css inside public folder) which are needed 
